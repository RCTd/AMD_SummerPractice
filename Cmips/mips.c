...
void PC(char*clk,char* pcIn,int pcOut,char* ctrlSignals);
...
void Exec(char* clk,char* signals,int exOut,char* ctrlSignals);
...
int main() {
    ...
    if(fork()>0){//(Control runs on this process)
        ...
        if (fork() > 0) {//(Exec runs on this process)
            ...
            close(ExPcPipe[0]);
            write(ExPcPipe[1], "00000000", hexWord + 1); //PC Load
            close(DeExPipe[1]);
            
            while ((nbytes = read(DeExPipe[0], DeExBuff, deOutBits + 1)) > 0) {
                printf("signalsbuff:%s\n", DeExBuff);
                
                Exec("1",DeExBuff,ExPcPipe[1],"11111111");
            }
            if (nbytes != 0) {
                exit(2);
            }
...
//                printf("addrout:%s\n", ImDeBuff);
                Decode("1", ImDeBuff, DeExPipe[1],DataCtrlPipe[1],"1");
            }
            if (nbytes != 0) {
                exit(2);
            }
        ...
        if (fork() > 0) {//(PC runs on this process)
            ...
            while ((nbytes = read(PcImPipe[0], PcImBuff, hexWord + 1)) > 0) {
                InstrMem("1", PcImBuff, ImDePipe[1],"111");
            }
            if (nbytes != 0) {
                exit(2);
            }
...
void Exec(char* clk,char* signals,int exOut,char* ctrlSignals)
{
    ...
    {
        i++;
        write(exOut,"00000000", hexWord + 1);
    }else
    {
        close(exOut);
    }
